...
//  weight = "1*neutrino_id==12+0*(neutrino_id!=12)";
  ...
  NInCuts_mu1R_all[7] = selection->Draw("mu_recoKE>>InCuts_mu_all7",weight*weightNCpi*(R1&&mulike&&onedecay&&eEnergyCut&&mudwallcut&&mutowallcut));
  NInCuts_mu1R_all[8] = selection->Draw("mu_recoKE>>InCuts_mu_all8",weight*weightNCpi*(R1&&mulike&&onedecay&&eEnergyCut&&mudwallcut&&nonfoll&&mutowallcut));
